...
if ( ! class_exists( "sc_simple_meta_tags" ) ) {
	...
	class sc_simple_meta_tags {
		...
		function sc_save_wonderful_metas( $post_id ) {
			...
			add_post_meta( $post_id, '_sc_m_description', $_POST['scmetadescription'], true ) or update_post_meta( $post_id, '_sc_m_description', $_POST['scmetadescription'] );

			add_post_meta( $post_id, '_sc_m_keywords', $_POST['scmetakeywords'], true ) or update_post_meta( $post_id, '_sc_m_keywords', $_POST['scmetakeywords'] );
		...
		function sc_show_on_website() {
			...
			if ( ! is_404() ) {
				...
				if ( is_page() || is_home() ) :
					...
					$post_id	= ( is_array( $categories ) && count( $categories ) ? get_option('page_for_posts') : $post->ID );
				
					if ( get_option( 'use_pages_meta_data' ) == 'on' ) :

						$is_implemeted 		= TRUE;

						$meta_title 		= ( get_post_meta( $post_id, '_sc_m_title', TRUE ) != '' ? get_post_meta( $post_id, '_sc_m_title', TRUE ) : get_option( 'page_meta_title' ) );

						$meta_description 	= ( get_post_meta( $post_id, '_sc_m_description', TRUE ) != '' ? get_post_meta( $post_id, '_sc_m_description', TRUE ) : get_option( 'page_meta_description' ) );

						$meta_keywords 		= ( get_post_meta( $post_id, '_sc_m_keywords', TRUE ) != '' ? get_post_meta( $post_id, '_sc_m_keywords', TRUE ) : get_option( 'page_meta_keywords' ) );
				...
				if ( $is_implemeted ) :
					...
					if ( $meta_title != '' ) :
						...
						echo	"\n" . '<title>' . htmlentities( stripslashes( $meta_title ), ENT_COMPAT, 'UTF-8' ) . '</title>' . "\n";

					else :

						echo	"\n" . '<title>' . esc_attr( get_bloginfo( 'name', 'display' ) ) . ' ' . wp_title( ' | ', FALSE ) .'</title>';

					endif;
					
					if ( $meta_description != '' ) :

						echo	"\n" . '<meta name="description" content="'. htmlentities( stripslashes( $meta_description ), ENT_COMPAT, 'UTF-8' ) .'" />' . "\n";
					
					endif;
					
					if ( $meta_keywords != '' ) :

						echo	"\n" . '<meta name="keywords" content="'. htmlentities( stripslashes( $meta_keywords ), ENT_COMPAT, 'UTF-8' ) .'" />' . "\n";
					
					endif;
					
				else :

					echo	"\n" . '<title>' . get_bloginfo( 'name', 'display' ) . wp_title( ' | ', FALSE ) . '</title>';
